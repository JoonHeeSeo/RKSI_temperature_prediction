...
seq_len    = SEQ_LEN
model = TCNModel(input_size=input_size, seq_len=seq_len,
                 num_channels=[32, 64, 128], kernel_size=2, dropout=0.0).to(device)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if val_loss < best_val:
        best_val = val_loss
        torch.save({'model_state': model.state_dict(), 'optimizer_state': optimizer.state_dict()}, CHECKPT)
...
print(f"\n▶ TCN Test MAE: {mae:.2f} ℃, RMSE: {rmse:.2f} ℃")
